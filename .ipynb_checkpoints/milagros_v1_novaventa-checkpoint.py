...
def eliminar_outliers(df_mes_ceros, sup, inf, n):
    ...
    df_acumulado = df_acumulado[['CODIGO',	
                                 'CONSECUTIVO', 
                                 'NUEVA_DEM']]
    df_periodo = df_acumulado.rename(columns={'NUEVA_DEM':'DEMANDA'})
    
    # Mostrar el DataFrame acumulado
    reporte_outliers = df_outliers[df_outliers['IS_OUTLIER'] == True]
    
    return df_periodo, df_outliers, reporte_outliers
...
def calcular_campañas_a_evaluar(df_sku, periodo_max_evaluacion, porc_eval):
    ...
    campañas_evaluar = min(periodo_max_evaluacion, math.ceil(largo_serie_tiempo * porc_eval))

    return campañas_evaluar


# In[13]:


def crear_rango_campañas(df_sku, campañas_evaluar):
...
def evaluar_y_generar_pms(df_mes, df_mes_ceros, lista_skus, 
    ...
    for i, sku in enumerate(tqdm(lista_skus, desc="Procesando SKUs")):
        ...
        #display(df_sku_fecha)
        #display(df_sku_fecha_ceros)
        # Evaluar el largo de la serie de tiempo y calcular meses a evaluar aper cada sku
        campañas_evaluar = calcular_campañas_a_evaluar(df_sku_fecha, periodo_max_evaluacion, porc_eval)
        #print('campañas_evaluar:',campañas_evaluar)
        # Crear el rango de fechas para cortar el set de datos de acuerdo con meses a evaluar
        rango_campañas = crear_rango_campañas(df_sku_fecha, campañas_evaluar)
        #print('rango_campañas:',rango_campañas)  
        # Tamaño de histórico n maximo y rango   
        n_max = max(2, len(df_sku_fecha) - campañas_evaluar)        
        ...
        for n in rango_n: 
            ...
            for fecha_corte in rango_campañas:
                ...
                #print('datos_forecast_demanda:')
                #display(datos_forecast_demanda)
...
#         #meses_evaluar = calcular_meses_a_evaluar(df_sku_fecha, periodo_max_evaluacion, porc_eval)
#         campañas_evaluar = calcular_campañas_a_evaluar(df_sku_fecha, periodo_max_evaluacion, porc_eval)
#         print(sku, 'campañas_evaluar:',campañas_evaluar)
#         # Crear el rango de fechas para corar el set de datos de acuerdo con meses a evaluar
#         #rango_fechas = crear_rango_fechas(df_sku_fecha, meses_evaluar)
#         rango_campañas = crear_rango_campañas(df_sku_fecha, campañas_evaluar)
#         print(rango_campañas)
#         
#         forecasts = []
#         # Iterar por fecha
#         for fecha_corte in rango_campañas:
...
#             print(sku, 'largo serie:',len(demand_series))
#             #print(demand_series)
...
#                         mes_inicial = fechas_futuras[0].index
#                     
...
#         else:
#             print("No hay SKUs con suficientes camapañas para un analisis estacional")
#             return None, None
#         #df_forecasts_mstl = pd.concat(df_lag_forecasts, ignore_index=True)
...
def obtener_mejor_pronostico(df_minimos, df_todos_pronosticos_fecha):
...
def crear_grafica_pronostico(df_mes, df_todos_pronosticos, df_pronosticos_mejor_modelo):
...
# df_orig = preprocesar_datos_1(df)
# df_vertical = llenar_nan(df_orig)
# df_resultado = eliminar_ceros_iniciales(df_vertical)
# df_ceros = reemplazar_ceros(df_resultado)
# graficar_demanda_codigo(df_resultado)
# sup = 0.98
# inf = 0.02
# n = 6
...
# 
# df_mejor_n, df_forecast_pms = evaluar_y_generar_pms(df_periodo, df_ceros, lista_skus, 
#                                                     periodo_max_evaluacion, 
#                                                     porc_eval, 
#                                                     meses_a_pronosticar_evaluacion,
#                                                    barra_progreso,
...
# df_forecast_final_pms = construir_pronostico_pms(df_mejor_n, df_periodo, meses_a_pronosticar_produccion, 'pms')
# 
# df_mejor_se,  df_forecast_se = encontrar_mejor_se(df_periodo, df_ceros, lista_skus, periodo_max_evaluacion, porc_eval, 
#                         meses_a_pronosticar_evaluacion,
#                         barra_progreso,
#                         status_text)
...
# df_mejor_se_final,  df_forecast_final_se = encontrar_mejor_se(df_periodo, df_ceros, lista_skus, periodo_max_evaluacion, porc_eval_pronost, 
...
# 
# porc_eval = 0.35
# df_mejor_rl_lineal, df_mejor_rl_estacional, df_forecast_rl_lineal, df_forecast_rl_estacional = aplicar_regresion_lineal_simple(lista_skus, df_periodo, df_ceros, 
#                                     periodo_max_evaluacion, porc_eval, 
#                                     meses_a_pronosticar_evaluacion,
#                                     barra_progreso,
#                                     status_text)
...
# 
# 
# df_final_mejor_rl_lineal, df_final_mejor_rl_estacional, df_forecast_final_rl_lineal, df_forecast_final_rl_estacional = aplicar_regresion_lineal_simple(lista_skus, df_periodo, df_ceros, 
...
# peso_ult_data = 0.08
# df_mejor_mstl, df_forecast_mstl = aplicar_mstl(lista_skus, df_periodo, df_ceros, 
#                                     periodo_max_evaluacion, porc_eval, 
#                                     meses_a_pronosticar_evaluacion, peso_ult_data,
#                                     barra_progreso,
#                                     status_text)
...
# tabla_final_pronost, df_forecast_final_mstl = aplicar_mstl(lista_skus, df_periodo, df_ceros, 
#                                     periodo_max_evaluacion, porc_eval_pronost, 
#                                     meses_a_pronosticar_produccion, 
#                                     peso_ult_data,
#                                     barra_progreso,
...
# df_pronosticos_mejor_modelo, df_pronosticos_12_meses = obtener_mejor_pronostico(df_minimos, df_todos_pronosticos_fecha)
# fig = crear_grafica_pronostico(df_periodo, df_todos_pronosticos_fecha, df_pronosticos_mejor_modelo)
# fig.show()
# 
...
with tabs[0]:
    ...
    if st.session_state.df is not None:
        ...
        if ruta_demanda is not None:        
            ...
            st.write("Datos cargados:")
            st.write(st.session_state.df.head())
            st.session_state.df_resultado = eliminar_ceros_iniciales(llenar_nan(preprocesar_datos_1(df)))
            st.session_state.df_ceros = reemplazar_ceros(st.session_state.df_resultado)
    ...
    if st.session_state.df_ceros is not None:
        ...
        if st.session_state.mostrar_grafica == "con_ceros":
            ...
            graficar_demanda_codigo(st.session_state.df_resultado)
        elif st.session_state.mostrar_grafica == "sin_ceros":
            st.subheader("Gráfica: Demanda Sin Ceros")
            graficar_demanda_codigo(st.session_state.df_ceros)
                ...
                if "df_periodo" in st.session_state and "df_ceros" in st.session_state:
                    ...
                    if st.button("Evaluar y Generar"):
                        ...
                        with st.spinner('Evaluando Promedio Móvil Simple:'):
                            df_mejor_n, df_forecast_pms = evaluar_y_generar_pms(st.session_state.df_periodo,
                        ...
                        with st.spinner('Evaluando Suavizacion Exponencial Simple:'):
                            df_mejor_se,  df_forecast_se = encontrar_mejor_se(st.session_state.df_periodo,
                                                                              st.session_state.df_ceros, 
                                                                              lista_skus, 
                                                                              st.session_state.periodo_max_evaluacion, 
                                                                              st.session_state.porc_eval, 
                        ...
                        with st.spinner('Generando Suavización Exponencial:'):
                            ...
                            df_mejor_se_final,  df_forecast_final_se = encontrar_mejor_se(st.session_state.df_periodo, 
                                                                                          st.session_state.df_ceros, 
                                                                                          lista_skus, 
                                                                                          st.session_state.periodo_max_evaluacion,
                        ...
                        with st.spinner('Evaluando MSTL:'):
                            peso_ult_data = 0.08 
                            df_mejor_mstl, df_forecast_mstl = aplicar_mstl(lista_skus, st.session_state.df_periodo, 
                                                                           st.session_state.df_ceros, 
                                                                           st.session_state.periodo_max_evaluacion, 
                                                                           st.session_state.porc_eval, 
                                                                           st.session_state.meses_a_pronosticar_evaluacion, 
                        ...
                        fig = crear_grafica_pronostico(st.session_state.df_periodo, st.session_state.df_todos_pronosticos_fecha, df_pronosticos_mejor_modelo)
